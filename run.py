...
# # nohup python run.py run.json > MLP.log 2>&1 &
...
#             load_from_cache_file=not data_args.overwrite_cache
#         )
#         data_set = {key:torch.tensor(dataset[:][key]) for key in dataset[0].keys() if key != 'content'}
...
#             logger.info("\ndev_data loss: {:.4f}  accuracy: {:.4f}".format(total_loss, total_correct/total_num))
#             model.train()
        ...
        for epoch in range(training_args.num_train_epochs):
            ...
            with torch.no_grad():
                for batch in eval_data_loader:
                    ...
                    metrics = compute_metrics((output.logits, input['labels']))
                    total_loss += output.loss
                    total_correct += metrics["accuracy"]
                    total_num += 1
