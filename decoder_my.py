...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        ...
        output = self.encode(src, src_mask)[0]
...
def make_model(
    config, src_vocab, tgt_vocab, N=6, d_model=768, d_ff=2048, h=8, dropout=0.1
):
...
def run_epoch(
    ...
    for i, batch in enumerate(data_iter):
        src = batch[0].to(model._model_device)
        src_mask = batch[2].to(model._model_device)
        tgt = batch[3][torch.tensor(list(range(src.size(0)))),batch[6],:].to(model._model_device)
        tgt_mask = torch.ones_like(tgt).to(model._model_device)
        out = model.forward(
            src, tgt, src_mask, tgt_mask
        )
        tgt_y = batch[3].to(model._model_device)
        labels = batch[6].to(model._model_device)
        candidate_masks = batch[-1].to(model._model_device)
        loss, loss_node, logits = loss_compute(out, tgt_y, labels, candidate_masks)
        ...
        if mode == "train" or mode == "train+log":
            ...
            train_state.tokens += src.shape[0]*4
            if i % accum_iter == 0:
                optimizer.step()
        ...
        total_loss += loss
        total_tokens += src.shape[0]*4
        tokens += src.shape[0]*4
        ...
        if i % 40 == 1 and (mode == "train" or mode == "train+log"):
            ...
            print(
                (
                    ...
                    + "| Tokens / Sec: %7.1f | Accuracy: %6.2f | Learning Rate: %6.1e"
                )
                % (i, n_accum, loss / src.shape[0]*4, tokens / elapsed, metrics["accuracy"],lr)
            )
            start = time.time()
            tokens = 0
        del loss
        del loss_node
    return total_loss / total_tokens, total_correct/total_num
...
class SimpleLossCompute:
    ...
    def __call__(self, outputs, candidates, labels, norm=16):
        prediction_scores = self.generator(outputs)
        norm = outputs.size(0)*4
        ...
        candidate_logits = batched_index_select(mask_prediction_scores, candidate_indices).squeeze(-1).reshape(prediction_scores.shape[0], 4, -1) # (Batch_size, 4, num_choices)
